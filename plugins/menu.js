...
"buttonText": " 𝙠𝙡𝙞𝙠 𝙙𝙞𝙨𝙞𝙣𝙞",
    ...
    let after = conn.menu.after || (conn.user.jid == global.conn.user.jid ? '' : `Dihosting oleh https://wa.me/${global.conn.user.jid.split`@`[0]}`) + defaultMenu.after
    let _text = [
      before,
    ...
    await conn.send2ButtonLoc(m.chat, await(await fetch(fla + teks)).buffer(), text.trim(), 'Made with ❤ by Putbotz' `aktif selama : ${uptime}\n${week} ${date}`, 'Pemilik Bot', `${_p}owner`, 'Donasi', `${_p}donasi`, m)
  } catch (e) {
